...
class BodyData(object):
	def __init__(self, bulletId, color, joints={}, initial_pos=[0,0,0], initial_rot=[0,0,0,1], initial_joint_state=None, joint_driver=JointDriver()):
		...
		self.index_joint_map      = {info.jointIndex: joint for joint, info in self.joints.items()}
		self.index_joint_map[-1]  = None
...
def vec_add(a, b):
	return VectorTuple(a[0] + b[0], a[1] + b[1], a[2] + b[2])

def vec_sub(a, b):
	return VectorTuple(a[0] - b[0], a[1] - b[1], a[2] - b[2])

def vec_scale(a, x):
	return VectorTuple(a.x * x, a.y * x, a.z * x)

def invert_transform(frame_tuple):
	temp = pb.invertTransform(list(frame_tuple.position), list(frame_tuple.quaterion))
	return FrameTuple(VectorTuple(*temp[0]), QuaternionTuple(*temp[1]))
...
class BulletSimulator(object):
	...
	def init(self, gravity=[0,0,-9.81], mode=pb.DIRECT):
		...
		pb.setAdditionalSearchPath(':'.join(self.resolver.ros_paths)) #used by loadURDF
	...
	def load_robot(self, urdf_path, pos=[0,0,0], rot=[0,0,0,1], joint_driver=JointDriver()):
		...
		self.bodies[robotId] = BodyData(robotBId, self.__gen_next_color(), joint_map, pos, rot, joint_initial, joint_driver)
	...
	def get_link_state(self, bodyId, link, compute_vel=0):
		...
		if link == None:
			...
			frame = FrameTuple(VectorTuple(*pos), QuaternionTuple(*quat))
			...
			return LinkState(FrameTuple(VectorTuple(*ls[0]), QuaternionTuple(*ls[1])),
							 FrameTuple(VectorTuple(*ls[2]), QuaternionTuple(*ls[3])),
							 FrameTuple(VectorTuple(*ls[4]), QuaternionTuple(*ls[5])),
							 zero_vector,
							 zero_vector)

	# @profile
	def get_link_state_sym(self, bodyId, link, compute_vel=0):
		ls = pb.getLinkState(self.bodies[bodyId].bulletId, self.bodies[bodyId].link_index_map[link], compute_vel)
		return LinkState(frame3_quaternion(ls[1][0], ls[1][1], ls[1][2], ls[1][3], point3(*ls[0])),
						 frame3_quaternion(ls[3][0], ls[3][1], ls[3][2], ls[3][3], point3(*ls[2])),
						 frame3_quaternion(ls[5][0], ls[5][1], ls[5][2], ls[5][3], point3(*ls[4])),
						 vec3(0,0,0),
						 vec3(0,0,0))
	...
	def get_AABB(self, bodyId, linkId=None):
		...
		return AABB(VectorTuple(*res[0]), VectorTuple(*res[1]))

	# @profile
	def get_AABB_sym(self, bodyId, linkId=None):
		res = pb.getAABB(self.bodies[bodyId].bulletId, self.bodies[bodyId].link_index_map[linkId])
	...
	def get_contacts(self, bodyA=None, bodyB=None, linkA=None, linkB=None):
		...
		return [ContactPoint(self.bulletIds_to_bodyIds[c[1]],                                   # Body A
							 ...
							 VectorTuple(*c[5]),                                                # Point on A
							 VectorTuple(*c[6]),                                                # Point on B
							 VectorTuple(*c[7]),                                                # Normal from B to A
							 c[8],                                                              # Distance
							 c[9])                                                              # Normal force
				for c in contacts]

	# @profile
	def get_contacts_sym(self, bodyA=None, bodyB=None, linkA=None, linkB=None):
		bulletA = self.bodies[bodyA].bulletId if bodyA != None else -1
		bulletB = self.bodies[bodyB].bulletId if bodyB != None else -1
		bulletLA = self.bodies[bodyA].link_index_map[linkA] if bodyA != None and linkA != None else -1
		bulletLB = self.bodies[bodyB].link_index_map[linkB] if bodyB != None and linkB != None else -1
		...
		return [ContactPoint(self.bulletIds_to_bodyIds[c[1]],                                   # Body A
							 ...
							 self.bodies[self.bulletIds_to_bodyIds[c[1]]].index_link_map[c[3]], # Link of A
							 self.bodies[self.bulletIds_to_bodyIds[c[2]]].index_link_map[c[4]], # Link of B
							 point3(*c[5]),                                                     # Point on A
							 point3(*c[6]),                                                     # Point on B
							 vec3(*c[7]),                                                       # Normal from B to A
							 c[8],                                                              # Distance
							 c[9])                                                              # Normal force
				for c in contacts]
	...
	# @profile
	def get_closest_points_sym(self, bodyA, bodyB, linkA=None, linkB=None, dist=0.2):
		bulletA = self.bodies[bodyA].bulletId
		bulletB = self.bodies[bodyB].bulletId
		bulletLA = self.bodies[bodyA].link_index_map[linkA] if bodyA != None and linkA != None else -1
		bulletLB = self.bodies[bodyB].link_index_map[linkB] if bodyB != None and linkB != None else -1
