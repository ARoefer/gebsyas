...
class SpatialRelations(object):
	...
	def on(cls, context, obj, location):
		...
		if DLCylinder.is_a(obj) or DLCube.is_a(obj):
			if DLCube.is_a(location):
				...
				inxy = max(abs(dot(l2o, x_col(location.pose))) - location.length * 0.5, 0) + max(abs(dot(l2o, y_col(location.pose))) - location.width * 0.5, 0)
	...
	def max_aabb_expr(cls, obj, frame):
		if DLCylinder.is_a(obj):
			cz = z_col(obj.pose)
			cylinder_up_if = vec3(dot(cz, x_col(frame)), dot(cz, y_col(frame)), dot(cz, z_col(frame)))
			alpha = sp.acos(abs(cylinder_up_if[2]))
			beta  = crappyAtan2(abs(cylinder_up_if[0]) + 0.0001, abs(cylinder_up_if[1]))
			cosA  = sp.cos(alpha)
			icosA = sp.sin(alpha)
			cosB  = sp.cos(beta)
			icosB = sp.sin(beta)
			...
			bz = z_col(obj.pose)
			box_up_if = vec3(dot(bz, x_col(frame)), dot(bz, y_col(frame)), dot(bz, z_col(frame)))
			alpha = sp.acos(abs(box_up_if[2]))
			beta  = crappyAtan2(abs(box_up_if[0]) + 0.0001, abs(box_up_if[1]))
			gamma = sp.acos(abs(dot(z_col(frame), y_col(obj.pose))))
			cosA  = sp.cos(alpha)
			icosA = sp.sin(alpha)
			cosB  = sp.cos(beta)
			icosB = sp.sin(beta)
