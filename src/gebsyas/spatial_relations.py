...
def max_aabb_expr(obj, frame):
    if DLCylinder.is_a(obj):
        ...
        bz = z_of(obj.pose)
        box_up_if = vector3(dot(bz, x_of(frame)), dot(bz, y_of(frame)), dot(bz, z_of(frame)))
        ...
        return vector3(obj.radius, obj.radius, obj.radius)
...
class DirectedSpatialRelations(object):
    ...
    def above(cls, context, obj_a, obj_b, observer):
        if DLRigidGMMObject.is_a(obj_a):
            mlgc = sorted(obj_a.gmm)[-1]
            pose_a = mlgc.pose

        elif DLRigidObject.is_a(obj_a):
            pose_a = obj_a.pose
        else:
            raise Exception('Above relation is only defined for rigid objects. However object A is not one.')

        if DLRigidGMMObject.is_a(obj_b):
            mlgc = sorted(obj_b.gmm)[-1]
            pose_b = mlgc.pose

        elif DLRigidObject.is_a(obj_b):
            pose_b = obj_b.pose
        else:
            raise Exception('Above relation is only defined for rigid objects. However object B is not one.')

        # THIS IS OVERKILL FOR THE MOMENT. AND BROKEN TOO!
        # obs_frame = observer.frame_of_reference
        # b_hdim_in_obs = max_aabb_expr(obj_b, obs_frame)

        # b2a = pos_of(pose_a) - pos_of(pose_b)
        # b2a_in_obs = vector3(dot(b2a, x_of(obs_frame)), dot(b2a, y_of(obs_frame)), dot(b2a, z_of(obs_frame)))
        ...
        # out = NonDirectedSpatialRelations.next_to(context, obj_a, obj_b, observer)
    ...
    def below(cls, context, obj_a, obj_b, observer):
        if not DLRigidObject.is_a(obj_a):
            raise Exception('Below relation is only defined for rigid objects. However object A is not one.')

        if not DLRigidObject.is_a(obj_b):
            raise Exception('Below relation is only defined for rigid objects. However object B is not one.')
...
class NonDirectedSpatialRelations(object):
    ...
    def next_to(cls, context, obj_a, obj_b, observer):
        ...
        d = norm(pos_of(pose_a) - pos_of(pose_b))
